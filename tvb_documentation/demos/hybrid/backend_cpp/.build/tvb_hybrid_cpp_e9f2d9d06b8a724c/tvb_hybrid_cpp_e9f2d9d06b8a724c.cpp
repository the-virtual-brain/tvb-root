...
struct SubnetModel_0 {
  ...
  static constexpr std::array<double, 1> kParam_Iext = { 3.1000000000000001 };
  static constexpr std::array<double, 1> kParam_Iext2 = { 0.45000000000000001 };
  static constexpr std::array<double, 1> kParam_Kf = { 0 };
  static constexpr std::array<double, 1> kParam_Ks = { 0 };
  static constexpr std::array<double, 1> kParam_Kvf = { 0 };
  static constexpr std::array<double, 1> kParam_a = { 1 };
  static constexpr std::array<double, 1> kParam_aa = { 6 };
  static constexpr std::array<double, 1> kParam_b = { 3 };
  static constexpr std::array<double, 1> kParam_bb = { 2 };
  static constexpr std::array<double, 1> kParam_c = { 1 };
  static constexpr std::array<double, 1> kParam_d = { 5 };
  static constexpr std::array<double, 1> kParam_r = { 0.00035 };
  static constexpr std::array<double, 1> kParam_slope = { 0 };
  static constexpr std::array<double, 1> kParam_tau = { 10 };
  static constexpr std::array<double, 1> kParam_tt = { 1 };
  static constexpr std::array<double, 1> kParam_x0 = { -1.6000000000000001 };
  ...
  static inline void compute_dfun(
    ...
    const double x1 = state(0, node, 0);
    const double y1 = state(1, node, 0);
    const double z = state(2, node, 0);
    const double x2 = state(3, node, 0);
    const double y2 = state(4, node, 0);
    const double g = state(5, node, 0);
    ...
    const double x0 = param_at(kParam_x0, node);
    const double Iext = param_at(kParam_Iext, node);
    const double slope = param_at(kParam_slope, node);
    const double Iext2 = param_at(kParam_Iext2, node);
    const double tau = param_at(kParam_tau, node);
    const double aa = param_at(kParam_aa, node);
    const double bb = param_at(kParam_bb, node);
    const double Kvf = param_at(kParam_Kvf, node);
    const double Kf = param_at(kParam_Kf, node);
    const double Ks = param_at(kParam_Ks, node);
    const double tt = param_at(kParam_tt, node);
    const double Coupling_Term_pop1 = coupling[0 * kNumNodes + node];
    const double Coupling_Term_pop2 = coupling[1 * kNumNodes + node];
    const double f1 = (x1 < 0 ? (((-a) * std::pow(x1, 2.0)) + (b * x1)) : ((slope - x2) + (0.59999999999999998 * std::pow((z - 4), 2.0))));
    const double zterm = (z < 0 ? ((-0.10000000000000001) * std::pow(z, 7.0)) : 0);
    const double h = ((4 * (x1 - x0)) + zterm);
    const double f2 = (x2 < (-0.25) ? 0 : (aa * (x2 + 0.25)));
    dx[0] = (tt * ((((y1 - z) + Iext) + (Kvf * Coupling_Term_pop1)) + (f1 * x1)));
    dx[1] = (tt * ((c - (d * std::pow(x1, 2.0))) - y1));
    dx[2] = (tt * (r * ((h - z) + (Ks * Coupling_Term_pop1))));
    dx[3] = (tt * (((((((-y2) + x2) - std::pow(x2, 3.0)) + Iext2) + (bb * g)) - (0.29999999999999999 * (z - 3.5))) + (Kf * Coupling_Term_pop2)));
    dx[4] = (tt * (((-y2) + f2) / tau));
    dx[5] = (tt * ((-0.01) * (g - (0.10000000000000001 * x1))));
  }
  ...
  static inline void compute_voi(
    ...
    voi[0] = (state(3, node, 0) - state(0, node, 0));
    voi[1] = state(2, node, 0);
  }
...
// subnetworks:
//   subnet: model=Epileptor, integrator=HeunDeterministic, nodes=3, modes=1, state_vars=6, coupling_vars=2
// projections: none
...
inline SimulationMetadata describe() {
  return SimulationMetadata{
      "tvb_hybrid_cpp_e9f2d9d06b8a724c",
      "e9f2d9d06b8a724c00fe5cf2f35f78a5468d4c40a123276442ec66c3cfd6c349",
      "0.3",
      "visualize_cpp_models_timeseries_Epileptor",
      kDt,
      1,
      0,
      0,
