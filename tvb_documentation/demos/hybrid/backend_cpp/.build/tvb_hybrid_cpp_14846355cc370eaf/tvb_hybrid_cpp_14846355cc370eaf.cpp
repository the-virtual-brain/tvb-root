...
// Subnet 0: g2d (Generic2dOscillator)
struct SubnetModel_0 {
  static constexpr double      kDt              = 0.10000000000000001;
  static constexpr std::size_t kNumNodes        = 5;
  ...
  static constexpr std::array<double, 1> kParam_I = { 0 };
  static constexpr std::array<double, 1> kParam_a = { -2 };
  static constexpr std::array<double, 1> kParam_alpha = { 1 };
  static constexpr std::array<double, 1> kParam_b = { -10 };
  static constexpr std::array<double, 1> kParam_beta = { 1 };
  static constexpr std::array<double, 1> kParam_c = { 0 };
  static constexpr std::array<double, 1> kParam_d = { 0.02 };
  static constexpr std::array<double, 1> kParam_e = { 3 };
  static constexpr std::array<double, 1> kParam_f = { 1 };
  static constexpr std::array<double, 1> kParam_g = { 0 };
  static constexpr std::array<double, 1> kParam_gamma = { 1 };
  ...
  static inline void compute_dfun(
    ...
    const double e = param_at(kParam_e, node);
    const double f = param_at(kParam_f, node);
    const double g = param_at(kParam_g, node);
    const double alpha = param_at(kParam_alpha, node);
    const double beta = param_at(kParam_beta, node);
    const double gamma = param_at(kParam_gamma, node);
    const double Coupling_Term = coupling[0 * kNumNodes + node];
    dx[0] = ((d * tau) * ((((((alpha * W) - (f * std::pow(V, 3.0))) + (e * std::pow(V, 2.0))) + (g * V)) + (gamma * I)) + (gamma * Coupling_Term)));
    dx[1] = ((d * (((a + (b * V)) + (c * std::pow(V, 2.0))) - (beta * W))) / tau);
  }
...
// subnetworks:
//   g2d: model=Generic2dOscillator, integrator=HeunDeterministic, nodes=5, modes=1, state_vars=2, coupling_vars=1
// projections: none
...
inline SimulationMetadata describe() {
  return SimulationMetadata{
      "tvb_hybrid_cpp_14846355cc370eaf",
      "14846355cc370eafe61effc22c2d896fa9f7232fefa58b37418626f1d93ee6cd",
      "0.3",
      "native_stimulus_execution_control",
      kDt,
      1,
      0,
      0,
