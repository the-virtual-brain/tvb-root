...
class SiibraBase:
    ...
    def init_siibra_params(self, atlas_name, parcellation_name, subject_ids):
        ...
        if atlas and not parcellation:
            ...
            parcellations = self.get_parcellations_for_atlas(atlas)
        ...
        if not atlas and parcellation:
            ...
            atlases = self.get_atlases_for_parcellation(parcellation)
        ...
        if not subject_ids:
            ...
            subject_ids = self.parse_subject_ids(subject_ids)

        return atlas, parcellation, subject_ids

    # ######################################## COMMON CONNECTIVITY METHODS #################################################
    def get_connectivity_component(self, parcellation, component):
    ...
    def get_hemispheres_for_regions(self, region_names):
    ...
    def get_regions_positions(self, regions):
    ...
    def filter_structural_connectivity_by_id(self, weights, tracts, subj_ids):
        """
    ...
    def create_tvb_structural_connectivity(self, weights_matrix, tracts_matrix, region_names, hemispheres, positions):
    ...
    def get_tvb_connectivities_from_kg(self, atlas=None, parcellation=None, subject_ids=None):
        """
        ...
        """
        atlas, parcellation, subject_ids = self.init_siibra_params(atlas, parcellation, subject_ids)
        connectivities = {}
        weights = self.get_connectivity_component(parcellation, 'weights')
        tracts = self.get_connectivity_component(parcellation, 'tracts')
        ...
        if subject_ids != 'all':
            weights, tracts = self.filter_structural_connectivity_by_id(weights, tracts, subject_ids)
        ...
        hemi = self.get_hemispheres_for_regions(region_names)
        positions = self.get_regions_positions(regions)
        ...
        for i in range(no_conns):
            ...
            conn = self.create_tvb_structural_connectivity(weights_matrix, tracts_matrix, region_names, hemi, positions)
    ...
    def filter_functional_connectivity_by_id(self, fcs, subj_ids):
        """
    ...
    def get_fc_name_from_file_path(self, path_to_file):
        """
    ...
    def create_tvb_connectivity_measure(self, siibra_fc, structural_connectivity):
        """
        ...
        fc_name = self.get_fc_name_from_file_path(siibra_fc.filename)
        title = conn_measure.__class__.__name__ + '_' + fc_name
        conn_measure.title = title

        return conn_measure

    def get_connectivity_measures_from_kg(self, atlas=None, parcellation=None, subject_ids=None,
                                          structural_connectivities=None):
        atlas, parcellation, subject_ids = self.init_siibra_params(atlas, parcellation, subject_ids)
        conn_measures = {}

        fcs = self.get_connectivity_component(parcellation, 'fc')
        ...
        if subject_ids != 'all':
            fcs = self.filter_functional_connectivity_by_id(fcs, subject_ids)
        ...
        for fc in fcs:
            ...
            conn_measure = self.create_tvb_connectivity_measure(fc, sc)
    ...
    def get_connectivities_from_kg(self, atlas=None, parcellation=None, subject_ids=None, compute_fc=False):
        """
        Compute the TVB Structural Connectivities and optionally Functional Connectivities for the selected subjects
        """
        import siibra
        conn_measures_dict = {}
        sc_dict = self.get_tvb_connectivities_from_kg(atlas, parcellation, subject_ids)

        if compute_fc:
            conn_measures_dict = self.get_connectivity_measures_from_kg(atlas, parcellation, subject_ids, sc_dict)
