...
class TestSimulationController(BaseTransactionalControllerTest):
    ...
    def test_set_cortex_without_local_connectivity(self):
        ...
        text_file = self.dataset.fetch_data('regionMapping_16k_76.txt')
    ...
    def set_eeg(self):
        eeg_sensors_file = self.dataset.fetch_data('eeg_unitvector_62.txt.bz2')
        eeg_sensors = TestFactory.import_sensors(self.test_user, self.test_project, eeg_sensors_file,
                                                 SensorTypesEnum.TYPE_EEG)

        surface_file = self.dataset.fetch_data('cortex_16384.zip')
        surface = TestFactory.import_surface_zip(self.test_user, self.test_project, surface_file,
                                                 SurfaceTypesEnum.CORTICAL_SURFACE, True)

        eeg_projection_file = self.dataset.fetch_data('projection_eeg_62_surface_16k.mat')
    ...
    def set_meg(self):
        meg_sensors_file = self.dataset.fetch_data('meg_brainstorm_276.txt')
        ...
        meg_projection_file = self.dataset.fetch_data('projection_meg_276_surface_16k.npy')
    ...
    def set_seeg(self):
        seeg_sensors_file = self.dataset.fetch_data('seeg_588.txt')
        ...
        seeg_projection_file = self.dataset.fetch_data('projection_seeg_588_surface_16k.npy')
