...
INCORRECT_PASS_ERROR = "Incorrect password"
PAGE_NR_ERROR = 'Invalid page number returned'
RET_ERROR = 'Not all members returned'
...
class TestUserService(TransactionalTestCase):
    ...
    def test_create_user_happy_flow(self):
        ...
        data = dict(username=TEST_USER, display_name=TEST_DISPLAY_NAME, password=hash_password(TEST_PASSWORD),
                    email=TEST_EMAIL, role=TEST_ROLE, comment="")
        ...
        assert inserted_user.password == hash_password(TEST_PASSWORD), INCORRECT_PASS_ERROR
        assert inserted_user.email == TEST_EMAIL, "The email inserted is not correct."
        assert inserted_user.role == TEST_ROLE, "The role inserted is not correct."
    ...
    def test_create_user_empty_password(self):
        ...
        """
        data = dict(username=TEST_USER, display_name=TEST_DISPLAY_NAME, password="", email=TEST_EMAIL, role=TEST_ROLE,
                    comment="")
    ...
    def test_create_user_no_password(self):
        ...
        """
        data = dict(username=TEST_USER, display_name=TEST_DISPLAY_NAME, email=TEST_EMAIL, role=TEST_ROLE, comment="")
        with pytest.raises(UsernameException):
            self.user_service.create_user(**data)
    ...
    def test_create_user_no_email(self):
        ...
        """
        data = dict(username="test_username", display_name=TEST_DISPLAY_NAME, password=TEST_PASSWORD, email="", role=TEST_ROLE,
                    comment="")
        with pytest.raises(UsernameException):
            self.user_service.create_user(**data)
    ...
    def test_reset_password_happy_flow(self):
        ...
        inserted_user = dao.get_user_by_name(TEST_USER)
        assert inserted_user.password == hash_password(TEST_PASSWORD), INCORRECT_PASS_ERROR
        reset_pass_data = dict(username=TEST_USER, email=TEST_EMAIL)
    ...
    def test_get_users_for_project(self):
        ...
        user_ids = []
        for i in range(5):
            user = model_project.User(TEST_USER + str(i), "test_user_no" + str(i), TEST_PASS, TEST_EMAIL2)
    ...
    def test_get_members_pages(self):
        ...
        for i in range(MEMBERS_PAGE_SIZE + 3):
            user = model_project.User("test_user_no" + str(i), "test_user_no" + str(i), TEST_PASS, TEST_EMAIL2)
            user = dao.store_entity(user)
            user_ids.append(user.id)
        ...
        assert len(all_users) == MEMBERS_PAGE_SIZE + 3, RET_ERROR
        assert pag == 2, PAGE_NR_ERROR
        ...
        assert len(all_users) == MEMBERS_PAGE_SIZE, RET_ERROR
        assert pag == 1, PAGE_NR_ERROR
