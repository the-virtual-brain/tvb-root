...
class BIDSDataBuilder:
    ...
    def init_paths(self):
        self.bids_file_base_dir = os.path.abspath(os.path.join(self.bids_root_dir, os.pardir))
        self.bids_file_name = os.path.split(os.path.normpath(self.bids_root_dir))[1]
        self.temp_bids_dir_name = self.bids_file_name + '-' + str(uuid.uuid4()).split("-")[4]
        self.temp_bids_zip_dir = os.path.join(self.bids_file_base_dir, self.temp_bids_dir_name) + '.zip'
    ...
    def get_connected_dependencies_paths(self, sub, paths_queue, json_files_processed, import_dependencies_paths):
        ...
        while len(paths_queue)!=0:
            ...
            if json_files_processed[path] is False:
                ...
                try:
                    json_data = json.load(open(self.get_abs_path(self.bids_root_dir, sub, path)))
                        ...
                        if json_data.get(possible_path_key) != None:
                            if isinstance(json_data[possible_path_key], list):
                                for path1 in json_data[possible_path_key]:
                                    computed_path = self.get_abs_path(self.bids_root_dir, sub, path1)
                                ...
                                computed_path = self.get_abs_path(self.bids_root_dir, sub, json_data[possible_path_key])
                                import_dependencies_paths.add(computed_path)
                                paths_queue.append(computed_path)
                                if computed_path not in json_files_processed:
                                    json_files_processed[computed_path] = False
    ...
    def get_data_files(self, import_dependencies_paths, sub):
        ...
        for p in import_dependencies_paths:
            ...
            for f in files_to_copy:
                data_files.add(self.get_abs_path(self.bids_root_dir, sub, os.path.join(file_dir, f)))
        return data_files
    ...
    def create_dataset_subjects(self):
        ...
        """
        if self.subject_build_data_check() is False: return

        self.init_paths()

        logger.info("Creating BIDS dataset for {}".format(self.bids_data_to_import))
        ...
        for sub in subject_folders:
            ...
            for file_name in sub_contents:
                file_path = self.get_abs_path(self.bids_root_dir, sub, os.path.join(sub_contents_path, file_name))
            ...
            final_paths_set = self.get_complete_paths(sub, paths_queue, json_files_processed, import_dependencies_paths, final_paths_set)
    ...
    def create_dataset_json_files(self):
        ...
        """
        if self.files_build_data_check() is False: return
    ...
    def get_complete_paths(self, sub, paths_queue, json_files_processed, import_dependencies_paths, final_paths_set):
        import_dependencies_paths = self.get_connected_dependencies_paths(sub, paths_queue, json_files_processed, import_dependencies_paths)
    ...
    def subject_build_data_check(self):
        ...
        if self.bids_data_to_import is None:
            ...
            logger.info("BIDS data to import from the directory is not provided")
            return False
        return True
