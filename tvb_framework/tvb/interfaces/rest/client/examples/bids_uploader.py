...
possible_paths = {
    BIDSImporter.COORDS_TOKEN: [],
    BIDSImporter.NET_TOKEN: ["CoordsRows", "CoordsColumns"],
    BIDSImporter.SPATIAL_TOKEN: ["CoordsRows", "CoordsColumns", "ModelEq", "ModelParam", "Network"],
    BIDSImporter.TS_TOKEN: ["CoordsRows", "CoordsColumns", "ModelEq", "ModelParam", "Network"],
...
BIDS_UPLOAD_CONTENT = BIDSImporter.TS_TOKEN
...
def upload_bids_data(tvb_client_instance, path):
    ...
    connectivity_dto = tvb_client_instance.get_operation_results(operation_gid)
...
def create_bids_dataset(bids_data_to_import, bids_root_dir):
    ...
    bids_file_base_dir = os.path.abspath(
        os.path.join(bids_root_dir, os.pardir))
    bids_file_name = os.path.split(os.path.normpath(bids_root_dir))[1]
    temp_bids_dir_name = bids_file_name + '-' + str(uuid.uuid4()).split("-")[4]
    temp_bids_zip_dir = os.path.join(
        bids_file_base_dir, temp_bids_dir_name) + '.zip'
    ...
    for sub in subject_folders:
        ...
        sub_contents_path = os.path.abspath(os.path.join(
            bids_root_dir, sub, bids_data_to_import.value))
        ...
        for file_name in sub_contents:
            file_path = get_abs_path(
                bids_root_dir, sub, os.path.join(sub_contents_path, file_name))
        ...
        while len(paths_queue) != 0:
            ...
            if json_files_processed[path] is False:
                ...
                try:
                    json_data = json.load(
                        open(get_abs_path(bids_root_dir, sub, path)))
                        ...
                        if json_data.get(possible_path_key) != None:
                            if isinstance(json_data[possible_path_key], list):
                                for path1 in json_data[possible_path_key]:
                                    computed_path = get_abs_path(
                                        bids_root_dir, sub, path1)
                                ...
                                computed_path = get_abs_path(
                                    bids_root_dir, sub, json_data[possible_path_key])
        ...
        for p in import_dependencies_paths:
            ...
            for f in files_to_copy:
                data_files.add(get_abs_path(
                    bids_root_dir, sub, os.path.join(file_dir, f)))
...
if __name__ == '__main__':
    ...
    # upload_bids_data(tvb_client, "C:/Users/upadh/Documents/GitHub/tvb-root/tvb_framework/tvb/interfaces/rest/client/examples/TS.zip")
