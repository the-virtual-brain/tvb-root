...
from tvb.datatypes.connectivity_mouse import MouseConnectivityCache
from tvb.datatypes.volumes import Volume


class MouseConnectivityCacheCreator(ABCUploader):
    "Creates a data cache for downloading Allen data."
    _ui_name = "Allen data cache creator."
    _ui_description = "Create a data cache for downloading mouse connectivity data from the Allen Institute."
    ...
    def get_upload_input_tree(self):
        return [{'name': 'resolution', 'type': 'select',
                 'label': 'Resolution of the data you want to download (microns):',
    ...
    def launch(self, resolution):
        tvb_mcc = MouseConnectivityCache(resolution=int(resolution), storage_path=self.storage_path)
        tvb_mcc.manifest_file=os.path.join(self.storage_path, 'manifest.json')
        # this ensures that the cache and thus the manifest file is actually
        # created for later use, though we don't use it immediately.
        allen_mcc = tvb_mcc.cache
        return tvb_mcc



class AllenConnectomeBuilder(ABCUploader):
    ...
    def get_upload_input_tree(self):
        return [# {'name': 'TransgenicLine', 'type': 'select', 'label': 'Transgenic line :',
                ...
                {'name': 'connectivitycache', 'label': 'Cache',
                 'type': MouseConnectivityCache, 'required': True, 'datatype': True,
                 'description': 'Allen mouse connectivity data cache.'},
    ...
    def launch(self, connectivitycache, resolution, weighting, inf_vox_thresh, vol_thresh):
        #tvb_mcc is the cache created before
        resolution = int(resolution)
        weighting = int(weighting)
        inf_vox_thresh = float(inf_vox_thresh)
        vol_thresh = float(vol_thresh)

        cache = connectivitycache.cache
        ...
        result_connectivity.storage_path = self.storage_path
...
# methods
def CacheCreator(resolution):
    resolution = int(resolution)
    tvb_mcc = MouseConnectivityCache(
        manifest_file='/dat1/francesca/TVB.svn/vmb_data/manifest.json',
        # os.path.join('/dat1/francesca/TVB.svn/', 'manifest.json'),
