...
from tvb.core.neocom.h5 import h5_file_for_view_model
...
def test_dummy_importer_mv_to_h5(tmph5factory):
    ...
    path = tmph5factory()
    h5_file = h5_file_for_view_model(divm, path)
    h5_file.store(divm)
    h5_file.close()
    ...
    h5_file = h5_file_for_view_model(loaded_divm, path)
...
def test_fourier_spectrum_model_to_h5(tmph5factory, time_series_index_factory):
    ...
    fs = FourierSpectrum(array_data=data, source=time_series, segment_length=10)

    fsm = FourierSpectrumModel(input_data=fs)
    fsm.input_data = FourierSpectrum()
    path = tmph5factory()
    h5_file = h5_file_for_view_model(type(fsm), path)

    h5_file.store(fsm)
    h5_file.input_data.store(fs.gid)
    h5_file.close()

    loaded_dt = FourierSpectrumModel()
    h5_file = h5_file_for_view_model(type(loaded_dt), path)
    h5_file.load_into(loaded_dt)
    input_data_gid = h5_file.input_data.load()
    assert input_data_gid == fs.gid
