...
class Simulator(HasTraits):
    ...
    def _loop_update_history(self, step, n_reg, state):
        ...
        if self.surface is not None and state.shape[1] > self.connectivity.number_of_regions:
            ...
            # state.data.strides = {tuple:3}(175360, 8, 8)
            # state.transpose((1, 0, 2)).data.strides = {tuple: 3}(8, 175360, 350720)
            # to get to next sv for a node i have to skip 175360 bytes in memory!
            add_at_313(region_state, self._regmap, state.transpose((1, 0, 2)))
