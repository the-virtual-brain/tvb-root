...
% for i in range(nl):
                    acc${i} += weights[i,j]*r[k, j, 256 + t - idelays[i, j], ${i}]
% endfor
% for i in range(nl):
                r_c${i} = g[k,${i}] * acc${i}
                r${i} = r[k, i, 256 + t, ${i}]
                V${i} = V[k, i, 256 + t, ${i}]
                r_noise${i} = r[k, i, 256 + t + 1, ${i}]
                V_noise${i} = V[k, i, 256 + t + 1, ${i}]
                dr${i} = rtau * (Delta_rpitau + 2 * V${i} * r${i})
                dV${i} = 1/tau * ( V${i}**2 - np.pi**2 * tau**2 * r${i}**2 + eta + J * tau * r${i} + I + r_c${i} ) 
                r[k, i, 256 + t + 1, ${i}] = r${i} + dt*dr${i} + r_noise${i}
                V[k, i, 256 + t + 1, ${i}] = V${i} + dt*dV${i} + V_noise${i}
% endfor
