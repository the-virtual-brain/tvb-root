...
class InterfaceCoSimulation(Raw):
    id_proxy = NArray(
        dtype=numpy.int,
    ...
    def __init__(self, **kwargs):
        super(InterfaceCoSimulation, self).__init__(**kwargs)
        self.coupling = None
        self.step = -1

    def config_for_sim(self, simulator):
        # configuration of all monitor
        super(InterfaceCoSimulation, self).config_for_sim(simulator)
        ...
        class HistoryProxy(simulator.history.__class__):
            n_proxy = Dim()
            ...
            # The creation of buffer for proxy because it's impossible to replace the data in the buffer of
            # state variable
            ...
            def __init__(self, weights, delays, cvars, n_mode):
                super(HistoryProxy, self).__init__(weights, delays, cvars, n_mode)
            ...
            def update_proxy(self, step, data):
                ...
                :param data: the date for the proxy node
                """
                if id_proxy.size != 0:
                    ...
                    self.buffer_proxy[indice] = numpy.reshape(data[1],
                                                              (indice.shape[0], self.n_cvar, self.n_proxy, self.n_mode))

            # query of the value of the proxy
            def query_proxy(self, step):
                return self.buffer_proxy[(step - 1) % self.n_time]

            # WARNING should be change if the function update of the history change
            # (the actual update is the same all history)
        ...
        index_no_cvar_make = numpy.array(
            [[i, j, k] for i in numpy.where(mask_no_cvar)[0] for j in id_proxy for k in
             range(simulator.model.number_of_modes)])

        class SimulatorProxy(type(simulator)):
            ...
            def __call__(self, simulation_length=None, random_state=None, proxy_data=None):
                ...
                if self.index_cvar.shape[0] != 0:
                    self.current_state[
                        (self.index_cvar[:, 0], self.index_cvar[:, 1], self.index_cvar[:, 2])] = numpy.squeeze(
                        self.history.query_proxy(self.current_step + 1))
                if self.index_no_cvar.shape[0] != 0:
                    self.current_state[
                        (self.index_no_cvar[:, 0], self.index_no_cvar[:, 1], self.index_no_cvar[:, 2])] = numpy.NAN

            def _loop_monitor_output(self, step, state):
                # modify the state variable before the record of the monitor
                if self.index_cvar.shape[0] != 0:
                    state[(self.index_cvar[:, 0], self.index_cvar[:, 1], self.index_cvar[:, 2])] = numpy.squeeze(
                        self.history.query_proxy(step + 1))
                if self.index_no_cvar.shape[0] != 0:
        ...
        simulator.__class__ = SimulatorProxy
