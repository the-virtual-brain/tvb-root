        ...
        # ICs provided
            ...
            if sim.surface is not None and n_node == nr:
                ...
                if ic_shape[0] >= sim.connectivity.horizon:
                    ...
                    if ic_shape[2] != sim.surface.region_mapping.shape[0] or ic_shape[1] != len(sim.model.state_variables):
                        raise ValueError(
                            'Incorrect initial condition shape for a surface sim. '
                            'Expected broadcastable to (time, n_state_vars, n_nodes, n_modes) = (1, %s, %s, %s). '
                            'Found %s' %
                            (len(sim.model.state_variables), sim.surface.region_mapping.shape[0], sim.model.number_of_modes,
                             ic_shape)
                        )
