from simulator.models.base import Model, ModelNumbaDfun
import numexpr
import numpy
from numpy import *
...
class Epileptor(ModelNumbaDfun):
    ...
    def _numpy_dfun(self, state_variables, coupling, local_coupling=0.0, ev=numexpr.evaluate):
        ...
        c_pop1 = coupling[0, :]
        c_pop2 = coupling[1, :]
        Iext = Iext = self.Iext + local_coupling * y[0]


        ev('tt * (y1 - z + Iext + Kvf * c_pop1 + (if_ydot0 + else_ydot0) )', out=derivative[0])
        ev('tt * (c - d * powf(x1, 2) - y1)', out=derivative[1])
        ev('tt * (r * ((ifmod_h + elsemod_h)) - z + Ks * c_pop1))', out=derivative[2])
        ev('tt * (-y2 + x2 - powf(x2, 3) + Iext2 + bb * g - 0.3 * (z - 3.5) + Kf * c_pop2)', out=derivative[3])
        ev('tt * (-y2 + else_ydot3) / tau', out=derivative[4])
    ...
    def dfun(self, vw, c, local_coupling=0.0):
        ...
        deriv = _numba_dfun_Epileptor(vw_, c_, self.a, self.b, self.c, self.d, self.r, self.s, self.x0, self.Iext, self.slope, self.Iext2, self.tau, self.aa, self.bb, self.Kvf, self.Kf, self.Ks, self.tt, self.modification, lc_0)

        return deriv.T[..., numpy.newaxis]

@guvectorize([(float64[:],) * 22], '(n),(m)' + ',()'*19 + '->(n)', nopython=True)
def _numba_dfun_Epileptor(vw, c_0, a, b, c, d, r, s, x0, Iext, slope, Iext2, tau, aa, bb, Kvf, Kf, Ks, tt, modification, lc_0, dx):
    ...
    dx[0] = tt * (y1 - z + Iext + Kvf * c_pop1 + (if_ydot0 + else_ydot0) )
    dx[1] = tt * (c - d * powf(x1, 2) - y1)
    dx[2] = tt * (r * ((ifmod_h + elsemod_h)) - z + Ks * c_pop1))
    dx[3] = tt * (-y2 + x2 - powf(x2, 3) + Iext2 + bb * g - 0.3 * (z - 3.5) + Kf * c_pop2)
    dx[4] = tt * (-y2 + else_ydot3) / tau
