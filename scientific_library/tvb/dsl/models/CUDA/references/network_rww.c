...
//            without some hooks and less tmps and only derivates
//            tmp_I_E = a_E * (w_E__I_0 + w_plus__J_NMDA * S_E + sum - JI*S_I) - b_E;
//            tmp_H_E = tmp_I_E/(1.0-exp(min_d_E * tmp_I_E));
//            tmp_I_I = (a_I*((w_I__I_0+(J_NMDA * S_E))-S_I))-b_I;
//            tmp_H_I = tmp_I_I/(1.0-exp(min_d_I*tmp_I_I));
//
//            sV = (imintau_E* S_E)+(tmp_H_E*(1-S_E)*gamma_E);
//            sW = (imintau_I* S_I)+(tmp_H_I*gamma_I);
//
//            S_E += dt*(sig * curand_normal(&s)+sV);
//            S_I += dt*(sig * curand_normal(&s)+Sw);
