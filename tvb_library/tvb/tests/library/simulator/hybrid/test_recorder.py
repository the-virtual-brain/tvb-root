...
class TestRecorder(BaseHybridTest):
    ...
    def test_temporal_average_monitor(self):
        ...
        step = 0
        while recorder._current_idx < 10:
    ...
    def _test_with_monitor(self, monitor_class=None, monitor_instance=None,
        ...
        if isinstance(monitor, Projection):
            ...
            if isinstance(monitor, EEG):
                monitor._ref_vec = np.zeros(
                    (monitor.sensors.number_of_sensors,))
                if monitor.reference:
                    if monitor.reference.lower() != 'average':
                        sensor_names = monitor.sensors.labels.tolist()
                        monitor._ref_vec[sensor_names.index(
                            monitor.reference)] = 1.0
                    else:
                        monitor._ref_vec[:] = 1.0 / \
                            monitor.sensors.number_of_sensors
                # Assume all sensors are usable and gain is finite for test
                monitor._ref_vec_mask = np.ones(
                    monitor.sensors.number_of_sensors, dtype=bool)
            ...
            if monitor.obsnoise is not None:
                if monitor.obsnoise.ntau > 0.0:
                    ...
                    monitor.obsnoise.configure_coloured(
                        dt=monitor.dt, shape=noiseshape)
                else:
        ...
        # Replace any existing monitors for this test
        net.monitors = [recorder]
        ...
        if expected_samples is not None:
            assert len(
                times) == expected_samples, f"Expected {expected_samples} samples, got {len(times)}"
    ...
    def test_with_raw(self):
        ...
        # 5.0/0.1 = 50 samples
        self._test_with_monitor(Raw, period=dt, expected_samples=50)

    def test_with_subsample(self):
        """Test recorder with SubSample monitor."""
        self._test_with_monitor(
            SubSample, period=0.5)  # Should give 10 samples for 5.0s simulation
    ...
    def test_with_hybrid_simulator(self):
        ...
        # Different period to test independence
        tavg2 = TemporalAverage(period=0.5)
        subnet1.add_monitor(tavg1)
        subnet1.monitors[0]._log = True
        subnet2.add_monitor(tavg2)

        assert id(subnet1.monitors) != id(subnet2.monitors)
    ...
    def test_with_rawvoi(self):
        ...
        times, samples = self._test_with_monitor(
            RawVoi, period=0.1)  # Same period as dt for raw
    ...
    def test_with_spatial_average(self):
        ...
        # Default VOI is usually 1 var for Generic2dOscillator
    ...
    def test_with_global_average(self):
        ...
        assert len(times) == 5
        # Global average reduces spatial dimension to 1
    ...
    def test_with_afferent_coupling_temporal_average(self):
        ...
        times, samples = self._test_with_monitor(
            AfferentCouplingTemporalAverage, period=1.0)
        assert len(times) == 5
        assert np.allclose(np.diff(times), 1.0)
    ...
    def _setup_projection_monitor(self, monitor_class, sensors_class):
        ...
        if sensors_class is SensorsMEG:
            ...
            sensor_kwargs['orientations'] = np.array(
        ...
        if monitor_class is EEG:
            ...
            raise ValueError(
                f"Unsupported monitor class for projection: {monitor_class}")
        ...
        monitor = monitor_class(
            period=1.0, sensors=sensors, projection=projection)
    ...
    def test_with_bold_region_roi(self):
        ...
        # Assuming dt=0.1 from _create_hybrid_simulator
        ...
        mock_region_mapping = np.array(
            [0, 0, 1, 1])  # Map 4 nodes to 2 regions
        ...
        times, samples = self._test_with_monitor(
            monitor_instance=monitor, simulation_length=10.0)
