...
class EpileptorRestingState(ModelNumbaDfun):
    ...
    state_variable_dfuns = Final(
        ...
        default={
            ...
            "x2": "tt * (-y2 + x2 - x2**3 + Iext2 + bb * g - 0.3 * (z - 3.5) + Kf * Coupling_Term_x2)",
            "y2": "tt * ((-y2 + (0.0 if x2 < -0.25 else aa * (x2 + 0.25))) / tau)",
            "g": "tt * (-0.01 * (g - 0.1 * x1))",
            "x_rs": "d_rs * tau_rs * (alpha_rs * y_rs - f_rs * x_rs**3 + e_rs * x_rs**2 + gamma_rs * I_rs + gamma_rs * K_rs * Coupling_Term_x_rs)",
    ...
    def _numpy_dfun(
        ...
        ydot[2] = self.tt * (
            ...
            * (
                4 * (y[0] - self.x0)
                - y[2]
                + where(y[2] < 0.0, if_ydot2, else_ydot2)
                + self.Ks * c_pop1
            )
        )
        ...
        ydot[6] = (
            ...
            * (
                ...
                - self.f_rs * y[6] ** 3
                + self.e_rs * y[6] ** 2
                + self.gamma_rs * self.I_rs
                + self.gamma_rs * self.K_rs * c_pop3
                + lc_1
            )
        )
        ydot[7] = (
            self.d_rs
            * (self.a_rs + self.b_rs * y[6] - self.beta_rs * y[7])
            / self.tau_rs
        )
