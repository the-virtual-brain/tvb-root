...
class Epileptor(ModelNumbaDfun):
    ...
    coupling_terms = Final(
        label="Coupling terms", default=["Coupling_Term_pop1", "Coupling_Term_pop2"]
    )
    ...
    dfun_intermediates = Final(
        ...
        default=[
            (
                "f1",
                "(-a * x1**2 + b * x1) if x1 < nb.float32(0.0) else (slope - x2 + nb.float32(0.6) * (z - nb.float32(4.0))**2)",
            ),
            (
                "zterm",
            ...
            (
                "f2",
                "nb.float32(0.0) if x2 < nb.float32(-0.25) else aa * (x2 + nb.float32(0.25))",
            ),
        ],
    )
    ...
    _nvar = 6
    cvar = numpy.array(
        [0, 3], dtype=numpy.int32
    )  # should these not be constant Attr's?
    ...
    def _numpy_dfun(
        ...
        if self.modification:
            h = self.x0 + 3.0 / (1.0 + numpy.exp(-(y[0] + 0.5) / 0.1))
        else:
            h = 4 * (y[0] - self.x0) + where(y[2] < 0.0, if_ydot2, else_ydot2)
...
class Epileptor2D(ModelNumbaDfun):
    ...
    parameter_names = List(
        of=str,
        label="List of parameters for this model",
        default="a b c d r x0 Iext slope Kvf Ks tt".split(),
    )
    ...
    dfun_intermediates = Final(
        ...
        default=[
            (
                "f1",
                "(a * x1**2 + (d - b) * x1) if x1 < nb.float32(0.0) else (-slope - nb.float32(0.6) * (z - nb.float32(4.0))**2 + d * x1)",
            ),
            (
                "zterm",
                "(nb.float32(-0.1) * z**7) if z < nb.float32(0.0) else nb.float32(0.0)",
    ...
    state_variable_dfuns = Final(
        label="Drift functions for numba codegen",
        default={
            "x1": "tt * (c - z + Iext + Kvf * Coupling_Term - f1 * x1)",
            "z": "tt * (r * (h - z + Ks * Coupling_Term))",
        },
    )

    state_variable_range = Final(
        default={"x1": numpy.array([-2.0, 1.0]), "z": numpy.array([2.0, 5.0])},
        label="State variable ranges [lo, hi]",
    ...
    def _numpy_dfun(
        ...
        if self.modification:
            ...
            h = 4 * (y[0] - self.x0) + where(y[1] < 0.0, if_ydot1, else_ydot1)
...
def _numba_dfun_epi2d(
    y, c_pop, x0, Iext, a, b, slope, c, d, r, Kvf, Ks, tt, modification, ydot
):
    ...
    if modification[0]:
        h = x0[0] + 3.0 / (1.0 + numpy.exp(-(y[0] + 0.5) / 0.1))
    else:
        h = 4 * (y[0] - x0[0]) + ydot[1]
