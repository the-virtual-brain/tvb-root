...
PYBIND11_MODULE(${module_name}, m) {
  ...
  m.def("debug_probe_history", []() {
    using tvb::hybrid::runtime::HistoryBuffer;
    using tvb::hybrid::runtime::StateBuffer;
    using tvb::hybrid::runtime::delayed_state_value;

    HistoryBuffer history(3, 1, 1, 1);
    StateBuffer s0(1, 1, 1);
    StateBuffer s1(1, 1, 1);
    StateBuffer s2(1, 1, 1);
    StateBuffer s3(1, 1, 1);
    s0(0, 0, 0) = 10.0;
    s1(0, 0, 0) = 20.0;
    s2(0, 0, 0) = 30.0;
    s3(0, 0, 0) = 40.0;
    ...
    out["helper_delay_0"] = delayed_state_value(history, 0, 0, 0, 0);
    out["helper_delay_1"] = delayed_state_value(history, 1, 0, 0, 0);
    out["helper_delay_2"] = delayed_state_value(history, 2, 0, 0, 0);
    return out;
  });
