...
class BaseSettingsProfile(object):
    ...
    def __init__(self):
        ...
        self.ELASTICSEARCH_URL = self.manager.get_attribute(stored.KEY_ELASTICSEARCH_URL)
        # The security key that is used to connect to the server
        self.ELASTICSEARCH_API_KEY = self.manager.get_attribute(stored.KEY_ELASTICSEARCH_API_KEY)
        # The request timeout for the elasticsearch rest calls
        self.ELASTICSEARCH_LOGGING_INDEX = self.manager.get_attribute(stored.KEY_ELASTICSEARCH_LOGGING_INDEX)
